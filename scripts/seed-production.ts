...
async function seedProduction() {
  ...
  try {
    // CRITICAL FIX: Check for invalid Chile buyer data (shouldn't exist)
    const chileId = "cmkl84b070004u6gkag39jtlk";
    const invalidChileMetrics = await prisma.buyerMetrics.count({
      where: { countryId: chileId }
    });
    if (invalidChileMetrics > 0) {
      console.log(`[FIX] Deleting ${invalidChileMetrics} invalid Chile buyer metrics (not in Excel)...`);
      await prisma.buyerMetrics.deleteMany({
        where: { countryId: chileId }
      });
      console.log("[FIX] Deleted invalid Chile data");
    }
    ...
    const needsMainSeed = countriesCount === 0;
    const needsBuyingSmmSeed = buyerMetricsCount === 0;

    if (!needsMainSeed && !needsBuyingSmmSeed) {
    ...
    // Main data seeding (countries, accounts, metrics)
    if (needsMainSeed) {
          ...
          try {
            await prisma.smmMetrics.upsert({
              ...
              create: {
                ...
                postsPlanDaily: metric.postsPlanDaily,
                postsFactDaily: metric.postsFactDaily,
                postsTotal: metric.postsTotal,
                postsRemaining: metric.postsRemaining,
                storiesPlan: metric.storiesPlan,
                storiesPlanDaily: metric.storiesPlanDaily,
                storiesFactDaily: metric.storiesFactDaily,
                storiesTotal: metric.storiesTotal,
                storiesRemaining: metric.storiesRemaining,
                miniReviewsPlan: metric.miniReviewsPlan,
                miniReviewsPlanDaily: metric.miniReviewsPlanDaily,
                miniReviewsFactDaily: metric.miniReviewsFactDaily,
                miniReviewsTotal: metric.miniReviewsTotal,
                miniReviewsRemaining: metric.miniReviewsRemaining,
                bigReviewsPlan: metric.bigReviewsPlan,
                bigReviewsPlanDaily: metric.bigReviewsPlanDaily,
                bigReviewsFactDaily: metric.bigReviewsFactDaily,
                bigReviewsTotal: metric.bigReviewsTotal,
                bigReviewsRemaining: metric.bigReviewsRemaining,
